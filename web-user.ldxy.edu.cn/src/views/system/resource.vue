<template>
    <section>
        ...
        <el-table :data="dataSet" highlight-current-row v-loading="listLoading" @selection-change="selsChange"
            ...
            <el-table-column prop="memo" label="描述" width="180" sortable>
            </el-table-column>
            <el-table-column prop="url" label="URL" width="320" sortable>
            </el-table-column>
            <el-table-column>
            </el-table-column>
        ...
        <!--编辑界面-->
        <el-dialog title="管理员信息编辑" v-model="editFormVisible" :close-on-click-modal="false">
        ...
        <!--新增界面-->
        <el-dialog title="新增资源URL" v-model="addFormVisible" :close-on-click-modal="false">
            <el-form :model="addForm" label-width="100px" :rules="addFormRules" ref="addForm">
                <el-form-item label="名称" prop="name">
                    <el-input v-model="addForm.name" auto-complete="off" placeholder="资源名称，不允许为空"></el-input>
                </el-form-item>
                <el-form-item label="描述" prop="memo">
                    <el-input v-model="addForm.memo" auto-complete="off" placeholder="资源描述，不允许为空"></el-input>
                </el-form-item>
                <el-form-item label="URL" prop="url">
                    <el-input  v-model="addForm.url" auto-complete="off" placeholder="URL路径，不允许为空"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
        ...
        methods: {
            ...
            handleDel: function (index, row) {
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    this.listLoading = true;
                    let para = row.id;
                    this.$http.post('/api/security/system/resource/delete',{
                        'id': para
                    }).then(response => {
...
/*                this.editForm.pwd = row.password;
            ...
            editSubmit: function () {
                this.$confirm('确认提交吗？', '提示', {}).then(() => {
                        ...
                        this.$http.post('/api/security/system/resource/add', {
                            'id': this.editForm.id,
